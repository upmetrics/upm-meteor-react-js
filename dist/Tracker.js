...
Tracker.Computation = class Computation {
   ...
   * @locus Client
   * @param {Function} f Function to be called on stop. Receives one argument, the computation that was stopped.
   */
  onStop(f) {
  ...
  invalidate() {
    ...
    if (!this.invalidated) {
      ...
      var _loop = function _loop(f) {
        Tracker.nonreactive(() => withNoYieldsAllowed(f)(_this));
      };
  ...
  stop() {
    ...
    if (!this.stopped) {
      ...
      var _loop2 = function _loop2(f) {
        Tracker.nonreactive(() => withNoYieldsAllowed(f)(_this2));
      };
...
Tracker.Dependency = class Dependency {
   ...
   * @locus Client
   * @param {Tracker.Computation} computation An optional computation declared to depend on `dependency` instead of the current computation.
   * @returns {Boolean}
   */
  depend(computation) {
    if (!computation) {
   ...
   */
  changed() {
    for (var id of Object.keys(this._dependentsById)) {
  ...
  try {
    ...
    if (pendingComputations.length || afterFlushCallbacks.length) {
      ...
      if (options.finishSynchronously) {
        console.error('still have more to do?'); // shouldn't happen
      }
 ...
 * rerunning.
 * @locus Client
 * @param {Tracker.ComputationFunction} f The function to run. It receives
...
Tracker.autorun = function (f, options) {
  ...
  var c = new Tracker.Computation(f, Tracker.currentComputation, o.onError);
  if (Tracker.active) {
 ...
 * @locus Client
 * @param {Function} f A function to call immediately.
 */
 ...
 * @locus Client
 * @param {Function} f A function to call at flush time.
 */
